...
def bimbam2rjped(samplePath, genoPath, cutoff, maf, sampleThreshold, markerThreshold, outputPath, sampleDropPath, markerDropPath, quiet, resume):
		...
		with open(genoPath,'rU') as genoFile:
			for line in genoFile:
				...
				for s in sampleRange:
					...
					f1 = float(line[4+2*s])
					f0 = 1.0 - f2 - f1
					if (f0 >= f2) and (f0 >= f1) and (f0 >= cutoff):
						markerGenotype[s] = 0
					elif (f1 >= f2) and (f1 >= f0) and (f1 >= cutoff):
						markerGenotype[s] = 1
					elif (f2 >= f1) and (f2 >= f0) and (f2 >= cutoff):
						markerGenotype[s] = 2
					else:
						markerGenotype[s] = -1
...
if __name__ == "__main__":
	versMaj,versMin,versRev,versDate = 0,1,0,'2012-02-03'
	version = "%d.%d.%d (%s)" % (versMaj, versMin, versRev, versDate)
	
	# define usage
	parser = argparse.ArgumentParser(
		formatter_class=argparse.RawDescriptionHelpFormatter,
		description="bimbam2rjped version %s" % version,
		epilog="""
example: %(prog)s -s samples.txt -b input.bimbam -c .85 -t 0.95 -o output.rjped
"""
	)
	...
	# run join
	bimbam2rjped(args.samples, args.bimbam, args.cutoff, args.maf, args.sample_threshold, args.marker_threshold, args.output, args.sample_drop, args.marker_drop, args.quiet, args.resume)
#__main__
